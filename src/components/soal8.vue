    ...
    <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden mt-20">
      <div class="p-8 space-y-6">
        <h1 class="text-3xl font-bold text-gray-900 mb-6">
          Data 8: Sertifikat Pelatihan/Seminar/Keikutsertaan Acara
        </h1>
        ...
        <div class="space-y-6 p-6">
          <div class="flex flex-col space-y-4">
            <div v-for="(field, index) in certificateFields" :key="index" class="flex flex-col gap-2">
              <hr v-if="['non_ipbi1', 'international1'].includes(field.key)" 
          ...
          <div class="flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4 sm:justify-between items-center mt-6">
            <router-link to="/soal-7" 
              ...
              @click.prevent="handleNavigation('/soal-7')">
              Kembali
            </router-link>
...
const fetchAnswer = async () => {
  try {
    loading.value = true;
    const response = await axios.get('/api/soal8', {
...
const deleteAllFiles = async () => {
  if (!confirm('Apakah Anda yakin ingin menghapus semua file nomor 8?')) return;
  
  try {
    loading.value = true;
    await axios.delete('/api/soal8', {
      headers: { Authorization: `Bearer ${authStore.accessToken}` }
    });
...
const viewFile = async (fieldName) => {
  try {
    const url = `/api/admin/soal/8/${authStore.user.id}/file/${fieldName}`;
