    ...
    <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden mt-20">
      <div class="p-8 space-y-6">
        <h1 class="text-3xl font-bold text-gray-900 mb-6">
          Data 12: Kepengurusan di Organisasi Ikatan Perangkai Bunga Indonesia (IPBI)
        </h1>
        ...
        <form @submit.prevent="submitAnswer" class="space-y-4">
          <div class="flex flex-col space-y-4">
            <div class="flex flex-col gap-2 p-2 bg-gray-50 rounded-md hover:bg-gray-100 transition-all duration-200">
              ...
              <select v-model="selectedJabatan" class="border p-2 rounded-md w-full sm:w-2/3 form-radio">
                <option value="" disabled>Pilih jabatan</option>
                <option value="inti">a. Pengurus Inti di DPP/DPD & DPC/LSK (10 poin)</option>
                <option value="biasa">b. Pengurus Biasa di DPP/DPD & DPC/LSK (5 poin)</option>
              </select>
              <p class="text-xs text-gray-500 mt-1">
                Note: Pengurus inti meliputi Ketua, Wakil Ketua, Bendahara, Sekretaris, Ketua Bidang (sedang menjabat saat ini)
              </p>

              <!-- Show saved status -->
              <div v-if="savedJabatan" class="text-green-600 flex items-center gap-2 mt-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                <span>Jabatan sudah tersimpan: {{ savedJabatan === 'inti' ? 'Pengurus Inti' : 'Pengurus Biasa' }}</span>
              </div>
            </div>
          </div>
...
const fetchAnswer = async () => {
  try {
    ...
    if (response.data && response.data.data && response.data.data.jabatan) {
      savedJabatan.value = response.data.data.jabatan;
      selectedJabatan.value = savedJabatan.value; // Sync with saved value
    } else {
      savedJabatan.value = '';
    }
  } catch (error) {
...
const submitAnswer = async () => {
  ...
  try {
    ...
    const endpoint = savedJabatan.value ? '/api/update12' : '/api/soal12';

    const response = await axios.post(endpoint, payload, {
      headers: { Authorization: `Bearer ${authStore.accessToken}` }
    });

    successMessage.value = response.data.message || 'Jabatan berhasil disimpan';
    savedJabatan.value = selectedJabatan.value;
    setTimeout(() => successMessage.value = '', 3000);
  } catch (error) {
    errorMessage.value = 'Gagal menyimpan jabatan: ' + (error.response?.data?.message || error.message);
  } finally {
    loading.value = false;
  }
};
...
const deleteData = async () => {
  ...
  try {
    ...
    successMessage.value = 'Data jabatan berhasil dihapus';
    setTimeout(() => successMessage.value = '', 3000);
  } catch (error) {
