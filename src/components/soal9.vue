<template>
      ...
      <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="p-8 space-y-6">
          <h1 class="text-3xl font-bold text-gray-900 mb-6">
            Soal 9: Simposium Ikatan Perangkai Bunga Indonesia (IPBI)
          </h1>
  
          <form @submit.prevent="submitAnswer" class="space-y-4">
            <div class="flex flex-col space-y-4">
              <div v-for="(field, index) in symposiumFields" :key="index">              
  ...
  const fetchAnswer = async () => {
      try {
          const response = await axios.get('http://localhost:8000/api/soal9', {
  ...
  const deleteAllFiles = async () => {
      try {
          await axios.delete('http://localhost:8000/api/soal9', {
              headers: { Authorization: `Bearer ${authStore.accessToken}` }
          });
  ...
  const submitAnswer = async () => {
      try {
          ...
          const endpoint = Object.keys(savedFiles.value).length > 0 
              ? 'http://localhost:8000/api/update9'
              : 'http://localhost:8000/api/soal9';
